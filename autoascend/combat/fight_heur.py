...
def elbereth_action(agent, monsters):
    ...
    if agent.blstats.hitpoints < 30 and adj_monsters_count > 0:
        ...
        # hypothesis: at critical HP, force Elbereth against any adjacent
        # non-weak threat with real combat level or high speed, while leaving
        # slow level-0 creatures to ordinary fighting.
        if agent.blstats.hitpoints <= 20 and any(
                mon[3].mname not in WEAK_MONSTERS + ONLY_RANGED_SLOW_MONSTERS and
                (mon[3].mlevel > 0 or mon[3].mmove >= 12) and
